"""Live FPB (First Pullback Breakout) scanner with ML filtering.

Runs on a 5-minute schedule during market hours:
  - 9:45 AM - 11:30 AM ET: scan for new entries + check exits
  - 11:30 AM - 3:55 PM ET: check exits only (trailing stop, target, time stop)

Fetches 1-min bars from IBKR, computes features matching the training pipeline,
scores with the trained LightGBM model, and creates paper trades for
high-probability FPB setups.

Strategy rules (matching backtester):
  - Filter: accum_phase IN (ACTIVE_ACCUM, LATE_ACCUM, EARLY_ACCUM), conviction >= 75
  - Setup: After 9:45 AM, detect bar high > or_high (OR breakout)
  - Entry: Within 60 min of breakout, price pulls back within 0.2% of or_high,
           then next bar closes above or_high
  - Stop: or_low
...
Usage:
    ...
    # Or test standalone:
    python -m signal_scanner.paper.fpb_live --test
"""
...
# Entry window (ET)
ENTRY_START_HOUR, ENTRY_START_MIN = 9, 45
ENTRY_END_HOUR, ENTRY_END_MIN = 11, 30

# Exit check window (ET) — wider than entry, to manage open positions
EXIT_START_HOUR, EXIT_START_MIN = 9, 45
EXIT_END_HOUR, EXIT_END_MIN = 15, 55

# FPB setup detection
PULLBACK_PROXIMITY_PCT = 0.2  # Pullback must come within 0.2% of OR high
PULLBACK_WINDOW_MIN = 60  # Pullback must occur within 60 min of breakout

# Intelligence filters
CONVICTION_MIN = 50
ACCUM_PHASES = {"ACTIVE_ACCUM", "LATE_ACCUM", "EARLY_ACCUM", "EXPANSION"}

# ML gate — SELECTIVE MODE (relaxed from P99 sniper that produced 0 trades)
ML_PROB_MIN = 0.50  # Raw prob floor (model still scores)
ML_PERCENTILE_MIN = 80  # Top 20% signals (was 99 = 0 trades ever)

# CANDLE FILTER (disabled — too restrictive, was blocking all entries)
SNIPER_REQUIRE_CANDLE = False  # Was True, N=28 sample too small to trust

# Position management — SNIPER (1-2 trades/day max)
MAX_FPB_POSITIONS = 15  # Paper mode — maximize trade data
...
# Trailing stop config — data-proven: trail 0.5R from peak = high expectancy
...
# ---------------------------------------------------------------------------
REC_SOURCE_PREFIX = "FPB_ML"


class FPBLiveScanner:
    """Live FPB (First Pullback Breakout) scanner with ML entry filtering.
    ...
    def run(self) -> None:
        ...
        if self._last_date != today:
            ...
            logger.info("FPB: new trading day — state reset")
        ...
        if self._model is None:
            self._load_model()
            if self._model is None:
                record_skip(Subsystem.FPB, SkipReason.MODEL_UNAVAILABLE)
                return
        ...
        # Always check exits on open FPB positions
        ...
        if not in_entry_window:
            record_skip(Subsystem.FPB, SkipReason.LATE_ENTRY_CUTOFF,
                         persist=False)
            return

        # Cap daily entries
        if len(self._entered_today) >= MAX_ENTRIES_PER_DAY:
            record_skip(Subsystem.FPB, SkipReason.POSITION_LIMIT,
                         f"daily cap {MAX_ENTRIES_PER_DAY}", persist=False)
            return

        # Cap open positions
        open_fpb = self._get_open_fpb_positions()
        if len(open_fpb) >= MAX_FPB_POSITIONS:
            record_skip(Subsystem.FPB, SkipReason.POSITION_LIMIT,
                         f"open cap {MAX_FPB_POSITIONS}", persist=False)
            return

        # Get qualifying tickers from intelligence snapshot
        tickers = self._get_qualifying_tickers()
        if not tickers:
            record_skip(Subsystem.FPB, SkipReason.NO_SETUP_QUALIFIED)
            logger.debug("FPB: no qualifying tickers from intelligence snapshot")
            return

        record_funnel(Subsystem.FPB, FUNNEL_CANDIDATES, len(tickers))
        ...
        for ticker in tickers:
            if ticker in self._entered_today:
                record_funnel(Subsystem.FPB, FUNNEL_SKIPPED)
                continue
            if len(open_fpb) + entered >= MAX_FPB_POSITIONS:
                break
            try:
                record_funnel(Subsystem.FPB, FUNNEL_ATTEMPTED)
                if self._scan_ticker(ticker, now_et):
                    record_funnel(Subsystem.FPB, FUNNEL_ENTERED)
                    entered += 1
            except Exception as e:
                logger.warning(f"FPB {ticker}: scan error — {e}")

        if entered:
            logger.info(f"FPB: {entered} new entries this scan cycle")
    ...
    def run_with_cache(self, bar_cache, ibkr_lock,
        ...
        for ticker in tickers:
            if _time.monotonic() - _t0 > budget_seconds:
                logger.info(f"FPB: budget {budget_seconds}s exhausted after {entered} entries")
                break
            if ticker in self._entered_today:
                record_funnel(Subsystem.FPB, FUNNEL_SKIPPED)
                continue
        ...
        if skipped_uncached:
            logger.debug(f"FPB: {skipped_uncached} tickers skipped (not in cache)")
        return entered
    ...
    def _load_model(self) -> None:
        """Load trained FPB LightGBM model from disk."""
        try:
            from signal_scanner.institutional_intel.config import WAREHOUSE_PATH

            model_path = WAREHOUSE_PATH.parent / "models" / "intraday_ml_fpb.pkl"
            if not model_path.exists():
                logger.warning(
                    f"FPB: model not found at {model_path}. "
            ...
            logger.info(
                f"FPB: loaded ML model (val AUC={val_auc:.4f}, "
                f"{len(self._feature_cols)} features)"
            )
        except Exception as e:
            logger.error(f"FPB: failed to load model — {e}")
            self._model = None
        ...
        try:
            ...
            try:
                ...
                if not row or not row[0]:
                    logger.warning("FPB: no daily prices in warehouse")
                    return
                ...
                logger.info(
                    f"FPB: loaded daily context for {len(self._daily_context)} tickers "
                    f"(as of {latest_date})"
                )
            finally:
                conn.close()
        except Exception as e:
            logger.warning(f"FPB: failed to load daily context — {e}")
    ...
    def _get_qualifying_tickers(self) -> List[str]:
        """Get tickers matching FPB criteria from intelligence snapshot."""
        snapshot = getattr(self._scanner, "_intelligence_snapshot", {})
        if not snapshot:
            logger.debug("FPB: intelligence snapshot empty")
            return []
        ...
        logger.debug(
            f"FPB: {len(qualified)} qualifying tickers "
    ...
    def _fetch_spy_bars(self) -> None:
        ...
        try:
            ...
            logger.debug(f"FPB: SPY bar fetch error — {e}")
    ...
    def _evaluate_setup(self, ticker: str, now_et: datetime,
        ...
        """Evaluate a ticker for FPB setup. Returns signal dict or None.

        Pure evaluation — no side effects. Called by strategy engine.
        Delegates to _scan_ticker with evaluate_only flag.
        """
    ...
    def _scan_ticker(self, ticker: str, now_et: datetime,
        ...
        """Scan a single ticker for FPB setup. Returns True if trade entered."""
        ...
        # 3. Check for FPB setup (breakout -> pullback -> re-entry)
        setup = self._check_fpb_setup(ticker, bars, or_high, or_low)
        if not setup:
            return False

        record_funnel(Subsystem.FPB, FUNNEL_SETUPS)
        ...
        if ml_prob is None or ml_prob < ML_PROB_MIN:
            logger.debug(
                f"FPB {ticker}: setup detected but ML prob {ml_prob:.3f} < {ML_PROB_MIN}"
            )
            return False

        # 4b. SNIPER GATE — ML percentile ranking (P99+)
        ml_percentile = self._compute_ml_percentile(ml_prob)
        if ml_percentile < ML_PERCENTILE_MIN:
            logger.debug(
                f"FPB {ticker}: ML percentile {ml_percentile} < {ML_PERCENTILE_MIN} — skipped"
            )
            return False

        # 4c. SNIPER GATE — Candle pattern (hammer or bullish engulfing)
        if SNIPER_REQUIRE_CANDLE:
            has_candle = self._detect_sniper_candles(bars, features)
            if not has_candle:
                logger.debug(
                    f"FPB {ticker}: no hammer/engulf candle detected — skipped"
                )
                return False

        logger.info(
            f"FPB SNIPER {ticker}: ALL GATES PASSED | "
            f"ML_pctl={ml_percentile} candle=OK"
        )

        # 5. Compute entry parameters
        entry_price = setup["entry_price"]
        stop_price = setup["stop_price"]

        if stop_price >= entry_price:
            # Stop above entry — invalid setup
            return False
        ...
        # No fixed target_2 — trailing stop handles the exit
        target_2 = entry_price + 10 * r_unit  # Very high — trail will exit before this
        rr_ratio = 2.0

        # 6. Grade computation (sniper mode = always SNIPER)
        grade = "SNIPER"
        ...
        if getattr(self, "_evaluate_only", False):
            self._last_signal = {
                "strategy": "FPB", "symbol": ticker, "side": "LONG",
            ...
            return True  # signal detected, no execution
        ...
        trade_data = {
            ...
            "recommendation_source": f"{REC_SOURCE_PREFIX}_SNIPER_P{ml_percentile}",
        ...
        # Route to IBKR if OrderExecutor is enabled for FPB
        if self._order_executor and self._order_executor.should_execute_live("FPB"):
            ...
            if placed:
                logger.info(f"FPB->IBKR: {ticker} bracket placed (trade_id={trade_id})")

        self._entered_today.add(ticker)

        logger.info(
            f"FPB ENTRY: {ticker} @ ${entry_price:.2f} "
            f"stop=${stop_price:.2f} target=${target_2:.2f} "
            f"ML={ml_prob:.1%} grade={grade} conv={intel.get('inst_conviction', 0):.0f} "
            f"(trade_id={trade_id})"
        )
        return True
    ...
    def _compute_features(
        ...
        """Compute FPB features from today's 1-min bars.
    ...
    # ------------------------------------------------------------------
    # FPB setup detection — THE CORE DIFFERENTIATOR
    # ------------------------------------------------------------------

    def _check_fpb_setup(
        self,
        ticker: str,
        bars: pd.DataFrame,
        or_high: float,
        ...
        """Detect First Pullback Breakout pattern.

        Phase 1: Find first bar where high > or_high after 9:45 (breakout)
        Phase 2: After breakout, find bar where low comes within 0.2% of or_high (pullback)
        Phase 3: After pullback, find bar closing above or_high (entry confirmation)
        All must happen within 60 minutes of breakout.

        Returns dict with entry_price, stop_price (or_low), entry_time, breakout_time
        if setup found, or None.
        """
        n = len(bars)
        if n < 15:
            return None
        ...
        # Bar timestamps — IBKR bars have datetime index
        bar_times = []
        for idx in bars.index:
            ts = pd.Timestamp(idx)
            bar_times.append(ts)

        # Phase 1: Find breakout (bar high > or_high after 9:45 = bar index 15)
        breakout_idx = None
        breakout_time = None
        or_bar_count = min(15, n)  # OR = first 15 bars

        for i in range(or_bar_count, n):
            ts = bar_times[i]
            h = ts.hour if hasattr(ts, "hour") else 0
            m = ts.minute if hasattr(ts, "minute") else 0

            # Only consider bars after 9:45 AM
            if h == 9 and m < 45:
                continue
        ...
        # Phase 2: After breakout, find pullback within 0.2% of or_high
        # within 60 minutes of breakout
        max_pullback_time = breakout_time + pd.Timedelta(minutes=PULLBACK_WINDOW_MIN)
        pullback_threshold = or_high * (1 + PULLBACK_PROXIMITY_PCT / 100.0)
        pullback_detected = False
        pullback_idx = None

        for i in range(breakout_idx + 1, n):
            ts = bar_times[i]
            if ts > max_pullback_time:
                break

            # Pullback: low comes within 0.2% of or_high (i.e. low <= or_high * 1.002)
            if not pullback_detected and lows[i] <= pullback_threshold:
                pullback_detected = True
                pullback_idx = i
                continue

            # Phase 3: After pullback, bar closing above or_high (entry confirmation)
            if pullback_detected:
                if closes[i] > or_high:
                    ...
                    if entry_price <= stop_price:
                        # Invalid — entry below or at stop
                        continue
    ...
    def _score_ml(self, features: Dict[str, Any]) -> Optional[float]:
        ...
        try:
            ...
            logger.debug(f"FPB ML scoring error: {e}")
            return None
    ...
    def _check_exits(self, now_et: datetime) -> None:
        """Check stop/trailing/time for open FPB positions.

        Trailing stop logic (data-proven with 0.5R trail):
        1. Initial stop: or_low
        ...
        """
        positions = self._get_open_fpb_positions()
        if not positions:
            return
        ...
        for pos in positions:
            ...
            # 3. Trailing stop management
                ...
                if not trail_activated and price >= target_1:
                    ...
                    logger.info(
                        f"FPB {ticker}: 1R HIT — trailing activated | "
    ...
    def _close_position(
        ...
        logger.info(
            f"FPB EXIT {ticker}: {reason} @ ${price:.2f} "
    ...
    def _detect_sniper_candles(
        self, bars: pd.DataFrame, features: Dict[str, Any]
    ) -> bool:
        """Detect hammer or bullish engulfing candles on 5-min bars.

        Returns True if at least one qualifying candle pattern found
        in the post-OR, pre-entry period.
        """
        ...
        # Aggregate to 5-min bars (every 5 1-min bars)
        hammer_found = False
        engulf_found = False
        prev_5m_body = 0.0

        for start in range(or_count, n - 4, 5):
            end = min(start + 5, n)
            o5 = opens[start]
            h5 = float(highs[start:end].max())
            l5 = float(lows[start:end].min())
            c5 = closes[end - 1]

            body = c5 - o5
            abs_body = abs(body)
            bar_range = h5 - l5
            lower_wick = min(o5, c5) - l5
            upper_wick = h5 - max(o5, c5)

            # Hammer: long lower wick, small upper wick
            if bar_range > 0 and lower_wick >= 2 * abs_body and upper_wick < abs_body:
                hammer_found = True

            # Bullish engulfing: current green engulfs prior red
            if body > 0 and prev_5m_body < 0:
                # Current green body covers prior red body
                prev_o = opens[max(start - 5, 0)]
                prev_c = closes[max(start - 1, 0)]
                if o5 <= prev_c and c5 >= prev_o:
                    engulf_found = True

            prev_5m_body = body

        return hammer_found or engulf_found

    # ------------------------------------------------------------------
    # Helpers
    # ------------------------------------------------------------------

    def _get_open_fpb_positions(self) -> List[Dict]:
        """Get open paper trades with FPB_ML source."""
...
def _test_main() -> None:
    ...
    parser = argparse.ArgumentParser(description="FPB Live Scanner Test")
    ...
    logger.info("FPB: standalone test mode")
    ...
    scanner = FPBLiveScanner(connector, db, mock_scanner)
    logger.info("Running FPB scan...")
    scanner.run()
    logger.info("Done")
